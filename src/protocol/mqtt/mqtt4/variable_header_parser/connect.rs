...
mod connect_flags_verify_tests {
    ...
    fn will_flag_false_then_will_qos_is_not_0_should_return_error() {
        ...
        assert!(matches!(
            result,
            Err(crate::protocol::mqtt::mqtt_protocol_error::MQTTProtocolError::MalformedPacket)
        ))
    }
    ...
    fn will_qos_more_than_2_should_return_error() {
        ...
        assert!(matches!(
            result,
            Err(crate::protocol::mqtt::mqtt_protocol_error::MQTTProtocolError::InvalidWillQoS(qos)) if qos == will_qos
        ))
    }
}
