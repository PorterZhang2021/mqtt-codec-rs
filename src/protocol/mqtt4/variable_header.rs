...
mod variable_header_tests {
    ...
    #[test]
    fn variable_header_can_parse_conn_ack_packet() {
        let fixed_header =
            FixedHeader::new(ControlPacketType::ConnAck, FixedHeaderFlags::ConnAck, 2);

        let mut bytes_mut = BytesMut::new();
        bytes_mut.write_a_byte(0b0000_0001); // session present = 1
        bytes_mut.write_a_byte(0x00); // return code = Connection Accepted
        ...
        assert_eq!(
            VariableHeader::ConnAck {
                conn_ack_variable_header
            },
            variable_header
        );
    }
    #[test]
    fn variable_header_can_parse_publish_packet() {
        ...
        let mut bytes_mut = BytesMut::new();
        write(&mut bytes_mut, "topic").unwrap();
        bytes_mut.write_a_byte(0x00); // Packet Identifier MSB
        bytes_mut.write_a_byte(0x0A); // Packet Identifier LSB (10)

        let variable_header = VariableHeader::parse(&fixed_header, &mut bytes_mut).unwrap();

        let publish_variable_header = PublishVariableHeader::new("topic".to_string(), Some(10));
        assert_eq!(
    ...
    #[test]
    fn variable_header_can_parse_pub_ack_packet() {
        let fixed_header = FixedHeader::new(ControlPacketType::PubAck, FixedHeaderFlags::PubAck, 2);
        ...
        let pub_ack_variable_header = PubAckVariableHeader::new(10);

        assert_eq!(
            VariableHeader::PubAck {
    ...
    #[test]
    fn variable_header_can_parse_pub_rec_packet() {
        let fixed_header = FixedHeader::new(ControlPacketType::PubRec, FixedHeaderFlags::PubRec, 2);
        ...
        let pub_rec_variable_header = PubRecVariableHeader::new(10);

        assert_eq!(
            VariableHeader::PubRec {
                pub_rec_variable_header
            },
            variable_header
        );
    }
    ...
    #[test]
    fn variable_header_can_parse_pub_rel_packet() {
        let fixed_header = FixedHeader::new(ControlPacketType::PubRel, FixedHeaderFlags::PubRel, 2);
        let mut bytes_mut = BytesMut::new();
        bytes_mut.write_a_byte(0x00); // Packet Identifier MSB
        bytes_mut.write_a_byte(0x0A); // Packet Identifier LSB (10)
        let variable_header = VariableHeader::parse(&fixed_header, &mut bytes_mut).unwrap();
        let pub_rel_variable_header = PubRelVariableHeader::new(10);
        assert_eq!(
            VariableHeader::PubRel {
                pub_rel_variable_header
            },
            variable_header
        );
    }
    ...
    #[test]
    fn variable_header_can_parse_pub_comp_packet() {
        let fixed_header =
            FixedHeader::new(ControlPacketType::PubComp, FixedHeaderFlags::PubComp, 2);
        let mut bytes_mut = BytesMut::new();
        bytes_mut.write_a_byte(0x00); // Packet Identifier MSB
        bytes_mut.write_a_byte(0x0A); // Packet Identifier LSB (10)
        let variable_header = VariableHeader::parse(&fixed_header, &mut bytes_mut).unwrap();
        let pub_comp_variable_header = PubCompVariableHeader::new(10);
        assert_eq!(
            VariableHeader::PubComp {
                pub_comp_variable_header
            },
            variable_header
        );
    }
    ...
    #[test]
    fn variable_header_can_parse_subscribe_packet() {
        let fixed_header =
            FixedHeader::new(ControlPacketType::Subscribe, FixedHeaderFlags::Subscribe, 2);
        let mut bytes_mut = BytesMut::new();
        bytes_mut.write_a_byte(0x00); // Packet Identifier MSB
        bytes_mut.write_a_byte(0x0A); // Packet Identifier LSB (10)
        let variable_header = VariableHeader::parse(&fixed_header, &mut bytes_mut).unwrap();
        let subscribe_variable_header = SubScribeVariableHeader::new(10);
        assert_eq!(
    ...
    #[test]
    fn variable_header_can_parse_sub_ack_packet() {
        let fixed_header = FixedHeader::new(ControlPacketType::SubAck, FixedHeaderFlags::SubAck, 2);
        let mut bytes_mut = BytesMut::new();
        bytes_mut.write_a_byte(0x00); // Packet Identifier MSB
        bytes_mut.write_a_byte(0x0A); // Packet Identifier LSB (10)
        let variable_header = VariableHeader::parse(&fixed_header, &mut bytes_mut).unwrap();
        let sub_ack_variable_header = SubAckVariableHeader::new(10);
        assert_eq!(
            VariableHeader::SubAck {
                sub_ack_variable_header
            },
            variable_header
        );
    }
    ...
    #[test]
    fn variable_header_can_parse_unsub_ack_packet() {
        let fixed_header =
            FixedHeader::new(ControlPacketType::UnsubAck, FixedHeaderFlags::UnsubAck, 2);
        let mut bytes_mut = BytesMut::new();
        bytes_mut.write_a_byte(0x00); // Packet Identifier MSB
        bytes_mut.write_a_byte(0x0A); // Packet Identifier LSB (10)
        let variable_header = VariableHeader::parse(&fixed_header, &mut bytes_mut).unwrap();
        let unsub_ack_variable_header = UnSubAckVariableHeader::new(10);
        assert_eq!(
            VariableHeader::UnsubAck {
                unsub_ack_variable_header
            },
            variable_header
        );
    }
    ...
    #[test]
    fn variable_header_can_parse_unsubscribe_packet() {
        let fixed_header = FixedHeader::new(
        ...
        bytes_mut.write_a_byte(0x0A); // Packet Identifier LSB (10)
        let variable_header = VariableHeader::parse(&fixed_header, &mut bytes_mut).unwrap();
        let unsubscribe_variable_header = UnSubScribeVariableHeader::new(10);
        assert_eq!(
    ...
    // todo pingreq
    #[test]
    fn variable_header_can_parse_ping_req_packet() {
        let fixed_header =
            FixedHeader::new(ControlPacketType::PingReq, FixedHeaderFlags::PingReq, 0);
        let mut bytes_mut = BytesMut::new();
        let variable_header = VariableHeader::parse(&fixed_header, &mut bytes_mut).unwrap();
        assert_eq!(VariableHeader::PingReq, variable_header);
    }

    #[test]
    fn variable_header_can_parse_ping_resp_packet() {
        let fixed_header =
            FixedHeader::new(ControlPacketType::PingResp, FixedHeaderFlags::PingResp, 0);
        let mut bytes_mut = BytesMut::new();
        let variable_header = VariableHeader::parse(&fixed_header, &mut bytes_mut).unwrap();
        assert_eq!(VariableHeader::PingResp, variable_header);
    }

    #[test]
    fn variable_header_can_parse_disconnect_packet() {
        let fixed_header = FixedHeader::new(
        ...
        let mut bytes_mut = BytesMut::new();
        let variable_header = VariableHeader::parse(&fixed_header, &mut bytes_mut).unwrap();
        assert_eq!(VariableHeader::Disconnect, variable_header);
    }
}
